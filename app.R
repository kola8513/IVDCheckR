#libraries
pacman::p_load(shiny,shinydashboard,flexdashboard,rhandsontable,fresh,shinydashboardPlus,shinyjs,rmarkdown,plotly,
               htmlwidgets,pagedown,pdftools,mcr,tinytex,shinycssloaders,knitr,digest,excelR,DBI)
...
# sanitize the inputs before passing them to any SQL queries 
# which patterns are allowed (i.e. whitelist) =  alphanumeric characters, dot, space, and hyphen
...
##check
# params:
#   params$projectInput <- sanitize_input(params$projectInput)
#   params$ParameterInfo <- sanitize_input(params$ParameterInfo)
#   params$ParameterUse <- sanitize_input(params$ParameterUse)
#   params$Indication <- sanitize_input(params$Indication)
#   params$ScientificValidity <- sanitize_input(params$ScientificValidity)
#   params$MeasurementScale <- sanitize_input(params$MeasurementScale)
#   params$SampleMaterial <- sanitize_input(params$SampleMaterial)
#   params$others_SampleMaterial <- sanitize_input(params$others_SampleMaterial)
#   params$RiskClass <- sanitize_input(params$RiskClass)
#   params$ModificationProduct <- sanitize_input(params$ModificationProduct)
#   params$SOPInput <- sanitize_input(params$SOPInput)
#   params$DeclarationInput <- sanitize_input(params$DeclarationInput)
#   params$AnalyticalInput <- sanitize_input(params$AnalyticalInput)
#   params$RiskInput <- sanitize_input(params$RiskInput)
#   params$ClinicalInput <- sanitize_input(params$ClinicalInput)
#   params$PostMarketInput <- sanitize_input(params$PostMarketInput)
#   params$extraText <- sanitize_input(params$extraText)


# restrict special inputs in the text
#https://stackoverflow.com/questions/38246439/restrict-input-type-in-shiny-field
...
cat("Raw data entered for quality control\n")
print(df)
...
server <- function(input, output, session) {
    ...
    output$exctable <- renderRHandsontable({
        rhandsontable(renamed_data,manualColumnResize = TRUE) |>
            hot_context_menu(allowRowEdit = F, allowColEdit = F,useTypes = FALSE) |> 
            onRender(jsCode)
    })
    ...
    observe({
        ...
        # Render the RMarkdown file into a Word report
        # temp_report_word <- file.path(temp_dir, "report.docx")
        #rmarkdown::render(temp_report_rmd, output_format = "word_document", output_file = temp_report_word)
        
        
        # word file option as a download
        # output$download_word_report <- downloadHandler(
        #   filename = function() {
        #     return("Report.docx")
        #   },
        #   content = function(file) {
        #     file.copy(temp_report_word, file)
        #   },
